...
require __DIR__ . '/config.php';
    ...
    if ($method === 'POST' && $action === 'signup') {
        ...
        $insert = $pdo->prepare('INSERT INTO users (name, email, phone, password_hash) VALUES (:name, :email, :phone, :password_hash)');
        $insert->execute([
            'name' => $name,
            'email' => $email,
    ...
    if ($method === 'POST' && $action === 'update-password') {
        $userId = currentUserId();
        if ($userId === null) {
        ...
        if ($newPassword === '') {
            respond(['error' => 'New password is required.'], 422);
        }

        if (strlen($newPassword) < 6) {
            respond(['error' => 'Password must be at least 6 characters long.'], 422);
        }
