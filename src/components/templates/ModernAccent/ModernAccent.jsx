...
const ModernAccent = ({ data, onSectionAction }) => {
  const {
    personalInfo = {},
    sectionHeadings = {},
  ...
  const h = {
    contact: sectionHeadings.contact || 'CONTACT INFO',
    ...
    technicalSummary: sectionHeadings.technicalSummary || 'TECHNICAL PROFILE',
  };
  ...
  const SectionControls = ({ region, index, total }) => (
    <div className="ma-controls no-print">
      <button
        type="button"
  ...
  const renderSectionBody = (entry, region) => {
    ...
    if (type === "contact") {
      return (
        <div className="ma-contact-list">
          ...
          {value.location && (
            <div className="ma-contact-item">
              <IconPin /> <span className="ma-pre-line">{value.location}</span>
            </div>
          )}
          {value.linkedin && (
            <div className="ma-contact-item">
      ...
      return (
        <div className="ma-tags">
          {(value.items || []).map((item, index) => (
            <span key={index} className="ma-tag">{item}</span>
          ))}
        </div>
      );
    }
    ...
    if (type === "technicalSummary") {
      ...
      return (
        <div className="ma-tech-list">
          {rows.map((row, idx) => (
            <div key={idx} className="ma-tech-item">
              <span className="ma-tech-label">{row.label}</span>
              <span className="ma-tech-val">{row.val}</span>
            </div>
          ))}
          {(value.additional || []).map((add, idx) => (
            <div key={`add-${idx}`} className="ma-tech-item">
              <span className="ma-tech-label">{add.label}</span>
              <span className="ma-tech-val">{add.value}</span>
            </div>
          ))}
        </div>
      );
    ...
    if (type === "experience") {
      return (
        <div className="ma-experience">
          {(value.items || []).map((job, index) => (
            <div key={index} className="ma-job-item">
              <div className="ma-job-header">
                <div className="ma-job-title-group">
                  <h3 className="ma-job-title">{job.title}</h3>
                  <div className="ma-job-company">{job.company} {job.location && `- ${job.location}`}</div>
                </div>
                <div className="ma-job-date">{job.duration}</div>
              </div>
              {job.achievements && job.achievements.length > 0 && (
                <ul className="ma-bullets">
    ...
    if (type === "education") {
      return (
        <div className="ma-education">
          {(value.items || []).map((item, index) => (
            <div key={index} className="ma-edu-item">
              <h3 className="ma-edu-degree">{item.degree}</h3>
              <div className="ma-edu-school">{item.school}</div>
              <div className="ma-edu-date">{item.year}</div>
            </div>
          ))}
        </div>
      );
      ...
      return (
        <div className="ma-projects">
          {(value.items || []).map((project, index) => (
            <div key={index} className="ma-project-item">
              <h3 className="ma-project-title">{project.title}</h3>
              {project.description && (
                <p className="ma-project-desc">{project.description}</p>
              )}
            </div>
          ))}
        </div>
      ...
      <header className="ma-header">
        <div className="ma-header-content">
          <div className="ma-header-text">
            <h1 className="ma-name">{personalInfo.fullName}<span className="ma-accent-dot">.</span></h1>
            <h2 className="ma-title">{personalInfo.title}</h2>
          </div>
          {personalInfo.image && (
            <div className="ma-photo-wrapper">
              <img src={personalInfo.image} alt={personalInfo.fullName} className="ma-photo" />
            </div>
          )}
        </div>
      </header>
      ...
      <div className="ma-body">
        <aside className="ma-sidebar">
          {(sectionLayout.sidebar || []).map((section, index) => (
            <section key={section.id} className="ma-section">
              <div className="ma-section-header">
                <h3 className="ma-section-title">{h[section.type]}</h3>
        ...
        <main className="ma-main">
          {personalInfo.summary && (
            <section className="ma-section">
              <div className="ma-section-header">
                <h3 className="ma-section-title">PROFILE</h3>
              </div>
              <p className="ma-summary">{personalInfo.summary}</p>
            </section>
          )}
