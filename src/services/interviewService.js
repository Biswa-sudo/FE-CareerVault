...
export const groqApiKey = import.meta.env.VITE_GROQ_API_KEY?.trim();
...
/**
 * Generates one AI batch of 10 topic questions with concepts.
 */
export async function generateTopicQuestions({ topic }) {
  if (!groqApiKey) {
    throw new Error('Missing Groq API key. Set VITE_GROQ_API_KEY in .env.local');
  }
      ...
      messages: [
        {
          role: 'system',
          content: [
            'You create interview-style practice sets.',
            'Return valid JSON only.',
            'Output shape: {"questions":[{"question":"...","concept":"..."}]}.',
            'Provide exactly 10 unique questions for the requested topic.',
            'Each concept must be a concise but correct explanation in 2-5 sentences.',
            'Do not include markdown code fences.'
          ].join(' ')
        },
        {
          role: 'user',
          content: `Generate a 10-question interview practice set for topic: ${topic}.`
        }
      ],
      temperature: 0.5,
      top_p: 0.9,
  ...
  const rawQuestions = normalizeQuestionList(parsed).slice(0, 10);

  if (rawQuestions.length < 10) {
    throw new Error('Groq did not return 10 questions. Please retry.');
  }

  return rawQuestions.map((entry, index) => ({
    id: `${topicSlug}-${createdAt}-${index + 1}`,
    topic,
